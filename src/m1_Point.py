...
class Point(object):
    ...
    def move_to(self, x, y):#distance formula
        self.howfarthepointmoved = math.sqrt((self.x-x)**2+(self.y-y)**2)+self.howfarthepointmoved
        self.x=x
        self.y=y
        self.rem = self.rem + 1

    def move_by(self,x,y):
        self.howfarthepointmoved=math.sqrt((x)**2+(y)**2)+self.howfarthepointmoved
        self.x=self.x+x
        self.y=self.y+y
        self.rem=self.rem+1
    def get_number_of_moves_made(self):
        return self.rem
    def get_distance_from(self,x):
        total = (self.x-x.x)**2 + (self.y-x.y)**2
        return math.sqrt(total)
    def get_distance_from_start(self):
        start=Point(self.startingx,self.startingy)
        total=self.get_distance_from(start)
        return total
    def get_distance_traveled(self):

        return self.howfarthepointmoved
    def closer_to(self,p2,p3):
        distancetwo=self.get_distance_from(p2)
        distancethree=self.get_distance_from(p3)
        if(distancetwo<=distancethree):
            return p2
        else:
            return p3
    def halfway_to(self,p2):
        halfway=(self.x+p2.x)/2
        halfway1=(self.y+p2.y)/2
        return halfway,halfway1
